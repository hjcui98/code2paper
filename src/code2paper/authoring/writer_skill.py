...
class PublicationMethodWriterSkillV1(BaseModel):
    ...
    style_rules: tuple[str, ...] = (
        ...
        "Treat section_candidate_points as an explicit candidate-narrative paragraph plan. Group related points into coherent paragraphs in argument-unit order, state each point once, and visibly preserve its caveat as a natural author-intent, partial-support, or mismatch statement. Candidate narrative is allowed in the editable candidate even when it cannot enter the repository-verified output.",
        ...
        "A candidate-only proposition must become substantive Method content under explicit natural author-intent, partial-support, or mismatch framing. Never replace it with a status label or an empty promise to discuss the point.",
    ...
    def system_prompt(self) -> str:
        lines = [
            ...
            "writer_view positive_propositions/concepts are repository-supported Method content; caveated propositions/concepts are candidate-only author narrative and must retain an explicit natural intended, partial-support, or mismatch caveat.",
            ...
            "When authoring_packets_v2 is present, it is the sole paragraph organization surface: follow ordered_targets, surface_mode, dossier summary, conditions, configuration state, and consumer-scoped formula packages. Return rendered ids only; the separate Binder supplies exact witnesses after prose is frozen.",
            "When paragraph_transaction_required is true, return exactly one paragraph transaction for each supplied paragraph_plan row. Put only that paragraph's substantive prose in paragraph_markdown (no H1/H2/H3 heading), declare the facet, slot, edge, and formula-package ids actually rendered, and let the separate Binder attach exact witnesses after the prose is frozen.",
            ...
            "Return only the structured section object. Copy section_id and every paragraph/slot/edge/formula-package id exactly from the closed contract; rendered_paragraph_ids, rendered_slot_ids, rendered_edge_ids, and used_formula_package_ids must name only content actually expressed in section_markdown. Copy proposition/brief/facet IDs exactly from their closed sets. Do not emit claim, fact, frame, equation, configuration, argument-unit, or rhetorical-move IDs except the closed binding fields requested by the response schema. The prose must never contain audit, callback, sidecar, pending, unverified, validation-status, repository-evidence, formalization-pending, or typed-gap terms.",
        ]
        return "\n".join(lines)
