"""Phase 3 Evidence: freeze method evidence for authoring.
...
from code2paper.pipeline.stages.evidence_fallback import raw_evidence_fallback_stage
...
def build_method_evidence(
    ...
    fallback_stage = raw_evidence_fallback_stage(raw_pack)
...
def write_phase3_artifacts(
    *,
    ...
    method_evidence.paper_module_aliases = _paper_module_aliases(
        raw_pack=raw_pack,
        code_method_analysis=code_method_analysis,
        code_facts=code_facts,
        paper_context_texts=_read_paper_alias_contexts(method_root),
    )
    if code_facts:
    ...
    _write_json(paths["evidence"], method_evidence.model_dump(mode="json"))
    _write_json(paths["claims"], claim_map.model_dump(mode="json"))
    paths["evidence_notes"].write_text(_method_evidence_review(method_evidence), encoding="utf-8")
    manifest = Phase3Manifest(
        ...
        inputs={
            "evidence_raw": "paper/method/evidence_raw.json",
            "alignment": "paper/method/alignment.json",
            "analysis": "paper/method/analysis.json" if code_method_analysis else "",
            "facts": "paper/method/facts.json" if code_facts else "",
        },
        outputs={
...
_ALIAS_PATTERN = re.compile(
    r"(?P<expansion>[A-Z][A-Za-z0-9][A-Za-z0-9+/_\-\s]{4,100}?)\s*\((?P<alias>[A-Z][A-Z0-9+\-]{1,14})\)"
)
_ALIAS_STOPWORDS = {
    "a",
    "an",
...
def _paper_module_aliases(
    *,
    raw_pack: RawEvidencePack,
    code_method_analysis: CodeMethodAnalysis | None,
    code_facts: dict | None,
    paper_context_texts: list[str] | None = None,
) -> list[PaperModuleAlias]:
    candidates = _paper_alias_candidates(
        raw_pack=raw_pack,
        code_facts=code_facts,
        paper_context_texts=paper_context_texts or [],
    )
    if not candidates:
        return []
    code_targets = _alias_code_targets(code_method_analysis=code_method_analysis, code_facts=code_facts)
    aliases: list[PaperModuleAlias] = []
    seen: set[str] = set()
    for candidate in candidates:
        match = _best_alias_target(candidate["expansion"], code_targets)
        if not match:
            continue
        ...
        aliases.append(
            PaperModuleAlias(
                ...
                source_ids=_dedupe(candidate["source_ids"]),
                evidence_span_ids=_dedupe(match["evidence_ids"]),
                confidence=ConfidenceLevel.HIGH if match["score"] >= 3 else ConfidenceLevel.MEDIUM,
            )
        )
    return aliases


def _read_paper_alias_contexts(method_root: Path) -> list[str]:
    paths = [
        method_root / "agent_workspace" / "code_intake" / "content" / "core_snippets.json",
        method_root / "agent_workspace" / "code_intake" / "content" / "code_sources.json",
        method_root / "author_summary.json",
    ]
    texts: list[str] = []
    for path in paths:
        if not path.exists():
        ...
        try:
            ...
            continue
        texts.extend(_iter_fact_text(payload))
    return texts


def _paper_alias_candidates(
    *,
    raw_pack: RawEvidencePack,
    code_facts: dict | None,
    paper_context_texts: list[str],
) -> list[dict]:
    candidates: list[dict] = []
    for item in raw_pack.evidence_items:
        if item.source_type.value not in {"author", "comment", "config"} and "readme" not in item.path.lower():
            continue
        candidates.extend(_extract_alias_candidates(item.content_summary, source_id=item.evidence_id))
    for index, text in enumerate(paper_context_texts):
        candidates.extend(_extract_alias_candidates(text, source_id=f"paper_context:{index}"))
    if code_facts:
        for index, text in enumerate(_iter_alias_safe_fact_text(code_facts)):
            candidates.extend(_extract_alias_candidates(text, source_id=f"code_facts:{index}"))
    result: list[dict] = []
    by_alias: dict[str, dict] = {}
    for candidate in candidates:
        key = candidate["alias"].lower()
        if key not in by_alias:
            by_alias[key] = candidate
            result.append(candidate)
        else:
            by_alias[key]["source_ids"] = _dedupe(by_alias[key]["source_ids"] + candidate["source_ids"])
    return result


def _extract_alias_candidates(text: str, *, source_id: str) -> list[dict]:
    found: list[dict] = []
    for match in _ALIAS_PATTERN.finditer(str(text)):
        alias = match.group("alias").strip()
        expansion = _clean_alias_expansion(match.group("expansion"))
        if not _valid_alias_candidate(alias, expansion):
            continue
        found.append({"alias": alias, "expansion": expansion, "source_ids": [source_id]})
    return found


def _clean_alias_expansion(value: str) -> str:
    text = " ".join(str(value).replace("\n", " ").split())
    for sep in (". ", "; ", ": "):
        if sep in text:
            text = text.rsplit(sep, 1)[-1]
    lowered = text.lower()
    for cue in ("called this the ", "called the ", "calls this the ", "calls this ", "known as ", "named ", "i.e., "):
        if cue in lowered:
            start = lowered.rindex(cue) + len(cue)
            text = text[start:]
            lowered = text.lower()
    return text.strip(" -:;,.`*_")


def _valid_alias_candidate(alias: str, expansion: str) -> bool:
    if len(alias) < 2 or len(alias) > 14 or not expansion:
        return False
    if len(expansion.split()) < 2:
        return False
    if alias.lower() in {"api", "cpu", "cuda", "gpu", "json", "yaml", "http", "https"}:
        return False
    module_terms = {
        "block",
        "branch",
    ...
    if not any(term in expansion.lower().split() for term in module_terms):
        return False
    return True


def _alias_code_targets(*, code_method_analysis: CodeMethodAnalysis | None, code_facts: dict | None) -> list[dict]:
    targets: list[dict] = []
    if code_method_analysis:
        for module in code_method_analysis.method_modules:
            name = " ".join(module.symbols) or module.paper_role or module.path
            role = module.paper_role
            text = " ".join([name, role, module.path, " ".join(module.symbols)])
            targets.append(
                {
                    "name": name,
                    "role": role,
                    "text": text,
                    "evidence_ids": module.evidence_span_ids,
                }
            )
            ...
            targets.append(
                {
                    ...
                    "text": " ".join([mechanism.name, mechanism.description, " ".join(mechanism.inputs), " ".join(mechanism.outputs)]),
                    "evidence_ids": mechanism.supporting_span_ids,
                }
            )
    if code_facts:
        for module in [item for item in code_facts.get("modules", []) if isinstance(item, dict)]:
            evidence_ids = _as_clean_str_list(module.get("evidence_refs")) or _as_clean_str_list(module.get("evidence_ids"))
            targets.append(
                {
                    "name": str(module.get("name") or "").strip(),
                    "role": str(module.get("role") or module.get("method_ref") or "").strip(),
                    "text": _target_text_from_mapping(module),
                    "evidence_ids": evidence_ids,
                }
            )
        for step in [item for item in code_facts.get("pipeline_steps", []) if isinstance(item, dict)]:
            evidence_ids = _as_clean_str_list(step.get("evidence_refs")) or _as_clean_str_list(step.get("evidence_ids"))
            targets.append(
                {
                    "name": str(step.get("name") or "").strip(),
                    "role": str(step.get("description") or "").strip(),
                    "text": _target_text_from_mapping(step),
                    "evidence_ids": evidence_ids,
                }
            )
    return [target for target in targets if target["text"].strip() and target["evidence_ids"]]


def _target_text_from_mapping(value: dict) -> str:
    parts: list[str] = []
    for key in ("name", "role", "method_ref", "key_logic", "description", "algorithm_ref"):
        if value.get(key):
            parts.append(str(value.get(key)))
    for key in ("symbols", "attributes", "methods", "involved_modules", "key_operations", "inputs", "outputs"):
        parts.extend(_as_clean_str_list(value.get(key)))
    return " ".join(parts)


def _best_alias_target(expansion: str, targets: list[dict]) -> dict | None:
    expansion_terms = _alias_terms(expansion)
    if not expansion_terms:
        return None
    module_like_expansion = any(
        term in expansion.lower().split()
        for term in {"block", "branch", "component", "decoder", "encoder", "head", "module", "network", "unit"}
    )
    best: dict | None = None
    for target in targets:
        target_terms = _alias_terms(target["text"])
        overlap = _soft_term_overlap(expansion_terms, target_terms)
        if overlap < 2:
            continue
        score = overlap
        normalized_expansion = _normalize_name(expansion)
        normalized_target = _normalize_name(target["text"])
        if normalized_expansion and normalized_expansion in normalized_target:
            score += 2
        if module_like_expansion and target_terms.intersection({"branch", "component", "decoder", "encoder", "head", "implementation", "module", "network", "projection", "unit"}):
            score += 1
        if module_like_expansion and target_terms.intersection({"loop", "objective", "optimizer", "pretraining", "training"}):
            score -= 2
        if best is None or score > best["score"]:
            best = {**target, "score": score}
    return best


def _alias_terms(text: str) -> set[str]:
    terms = set()
    for raw in re.findall(r"[A-Za-z][A-Za-z0-9+\-]*", str(text).lower()):
        token = raw.strip("-+")
        if len(token) < 3 or token in _ALIAS_STOPWORDS:
            continue
        terms.add(token)
    return terms


def _soft_term_overlap(left: set[str], right: set[str]) -> int:
    count = 0
    for term in left:
        if term in right or any(other.startswith(term[:5]) or term.startswith(other[:5]) for other in right if len(other) >= 5):
            count += 1
    return count


def _iter_fact_text(value: object):
    if isinstance(value, dict):
        for item in value.values():
            yield from _iter_fact_text(item)
...
def _iter_alias_safe_fact_text(value: object):
    if not isinstance(value, dict):
        return
    for key in ("author_alignment", "author_intents", "key_insights", "diagram_hints"):
        if key in value:
            yield from _iter_fact_text(value[key])
...
def _stage_packets_from_code_facts(
    *,
    ...
    for stage in method_evidence.stages:
        step = step_by_name.get(_normalize_name(stage.name)) or _best_matching_pipeline_step(
            stage.name,
            pipeline_steps,
        )
        candidate = candidate_by_name.get(_normalize_name(stage.name)) or _best_matching_candidate_mechanism(
            stage.name,
            code_method_analysis,
        )
...
def _analysis_ordered_method_stages(code_method_analysis: CodeMethodAnalysis | None) -> list[MethodStageAlignment]:
    ...
    for index, name in enumerate(proposed):
        mechanism = mechanisms_by_name.get(_normalize_name(name)) or _best_matching_candidate_mechanism(
            name,
            code_method_analysis,
        )
        ...
        stages.append(
            MethodStageAlignment(
                ...
                name=name,
                purpose=purpose or _default_author_stage_purpose(name),
...
def _purpose_for_analysis_stage(stage_name: str, code_method_analysis: CodeMethodAnalysis) -> str:
    ...
    matched_mechanism = _best_matching_candidate_mechanism(stage_name, code_method_analysis)
    if matched_mechanism:
        return matched_mechanism.description
    for flow in code_method_analysis.execution_flows:
...
def _default_author_stage_purpose(stage_name: str) -> str:
    normalized = _normalize_name(stage_name)
    if "overview" in normalized:
        return "Summarize the method flow and its evidence-backed components."
    if "training objective" in normalized or "loss" in normalized:
        return "Define the language-modeling objective only when implementation evidence is available."
    return f"Represent the paper-facing stage named {stage_name}."


def _analysis_stage_io(*, code_method_analysis: CodeMethodAnalysis | None, stage_name: str) -> dict[str, list[str]]:
    if not code_method_analysis:
        return {"inputs": [], "outputs": []}
    mechanism = _best_matching_candidate_mechanism(stage_name, code_method_analysis)
    if mechanism:
        return {
            "inputs": _clean_analysis_io_items(mechanism.inputs),
            "outputs": _clean_analysis_io_items(mechanism.outputs),
...
def _analysis_mechanisms_for_stage(
    *,
    ...
    if not candidates:
        best_candidate = _best_matching_candidate_mechanism(
            method_stage.name,
            code_method_analysis,
            preferred_evidence=stage_evidence,
        )
...
_STAGE_TERM_GROUPS: tuple[set[str], ...] = (
    ...
    {"interaction", "aware", "transformer", "compose", "composition"},
    {"dual", "branch", "score", "scoring", "logit", "logits", "loss", "objective", "optimization", "optimize", "regularized", "regularization", "training"},
    {"task", "aware", "evaluation", "evaluate", "inference", "predict", "prediction", "protocol", "report", "collection", "b2n", "fs", "cd"},
)

_STAGE_TERM_SYNONYMS: dict[str, set[str]] = {
    "configuration": {"config", "setup", "assembly", "launcher", "override"},
    "setup": {"configuration", "config", "assembly", "loading"},
    "backbone": {"encoder", "loading", "setup"},
    "shared": {"common", "representation"},
    "representation": {"semantic", "shared"},
    "projection": {"project", "aligner", "align"},
    "interaction": {"compose", "composition", "fusion", "transformer"},
    "transformer": {"interaction"},
    "training": {"optimization", "objective", "loss", "regularized"},
    "objective": {"optimization", "loss", "training", "regularized"},
    "optimization": {"objective", "loss", "training", "regularized"},
    "inference": {"evaluation", "prediction", "protocol", "report"},
    "evaluation": {"inference", "prediction", "report", "collection"},
    "protocol": {"task", "evaluation", "inference"},
}

_GENERIC_STAGE_TERMS = {
    "method",
    "overview",
    "paper",
    "facing",
...
def _best_matching_candidate_mechanism(
    stage_name: str,
    code_method_analysis: CodeMethodAnalysis | None,
    *,
    preferred_evidence: set[str] | None = None,
) -> CandidateMechanism | None:
    if not code_method_analysis:
        return None
    best: CandidateMechanism | None = None
    best_score = 0.0
    for mechanism in code_method_analysis.candidate_mechanisms:
        score = _stage_match_score(
            stage_name,
            mechanism.name,
            extra_text=mechanism.description,
        )
        if preferred_evidence and preferred_evidence.intersection(mechanism.supporting_span_ids):
            score += 0.2
        if score > best_score:
            best = mechanism
            best_score = score
...
def _best_matching_pipeline_step(stage_name: str, pipeline_steps: list[dict]) -> dict:
    best: dict | None = None
    best_score = 0.0
    for step in pipeline_steps:
        ...
        score = _stage_match_score(
            ...
            extra_text=str(step.get("description") or ""),
        )
        if score > best_score:
            best = step
            best_score = score
...
def _stage_match_score(stage_name: str, candidate_name: str, *, extra_text: str = "") -> float:
    stage_norm = _normalize_name(stage_name)
...
def _stage_match_core(stage_norm: str, candidate_text: str) -> float:
    candidate_norm = _normalize_name(candidate_text)
    if not stage_norm or not candidate_norm:
        return 0.0
    if stage_norm == candidate_norm:
        return 1.0
    if stage_norm in candidate_norm or candidate_norm in stage_norm:
        return 0.92

    stage_terms = _expanded_stage_terms(stage_norm)
    candidate_terms = _expanded_stage_terms(candidate_norm)
    if not stage_terms or not candidate_terms:
        return SequenceMatcher(None, stage_norm, candidate_norm).ratio()

    overlap = len(stage_terms & candidate_terms) / max(1, min(len(stage_terms), len(candidate_terms)))
    seq_ratio = SequenceMatcher(None, stage_norm, candidate_norm).ratio()
    group_score = _stage_group_overlap_score(stage_terms, candidate_terms)
    score = max(seq_ratio * 0.55 + overlap * 0.45, overlap, group_score)

    if _looks_like_training_stage(stage_terms) and _looks_like_training_stage(candidate_terms):
        score += 0.08
    if _looks_like_inference_stage(stage_terms) and _looks_like_inference_stage(candidate_terms):
        score += 0.08
    if _looks_like_config_stage(stage_terms) and _looks_like_config_stage(candidate_terms):
        score += 0.08
    if _looks_like_representation_stage(stage_terms) and _looks_like_representation_stage(candidate_terms):
        score += 0.08
    if _looks_like_transformer_stage(stage_terms) and _looks_like_transformer_stage(candidate_terms):
        score += 0.08

    return min(score, 1.0)


def _expanded_stage_terms(text: str) -> set[str]:
    base_terms = {
        part
        for part in _normalize_name(text).split()
        if part and part not in _GENERIC_STAGE_TERMS
    }
    expanded = set(base_terms)
    for term in list(base_terms):
        if term in _STAGE_TERM_SYNONYMS:
            expanded.update(_STAGE_TERM_SYNONYMS[term])
        for anchor, synonyms in _STAGE_TERM_SYNONYMS.items():
            if term in synonyms:
                expanded.add(anchor)
...
def _stage_group_overlap_score(stage_terms: set[str], candidate_terms: set[str]) -> float:
    best = 0.0
    for group in _STAGE_TERM_GROUPS:
        stage_hits = len(stage_terms & group)
        candidate_hits = len(candidate_terms & group)
        if stage_hits and candidate_hits:
            best = max(best, min(stage_hits, candidate_hits) / max(1, min(len(group), 3)))
    return best


def _looks_like_training_stage(terms: set[str]) -> bool:
    return bool(terms & {"training", "objective", "optimization", "regularized", "loss", "branch", "scoring"})
...
def _looks_like_config_stage(terms: set[str]) -> bool:
    return bool(terms & {"configuration", "config", "setup", "assembly", "launcher", "override"})


def _looks_like_representation_stage(terms: set[str]) -> bool:
    return bool(terms & {"shared", "representation", "projection", "aligner", "align"})
...
def _allow_support_module_in_stage(role: AlignedModuleRole, stage: MethodStageAlignment | None) -> bool:
    if stage is None:
        return False
    stage_terms = {
        term for term in _normalize_name(stage.name).split() if term and term not in _GENERIC_STAGE_TERMS
    }
    role_terms = {
        term
        for term in _normalize_name(f"{role.path} {role.symbol} {role.role}").split()
        if term and term not in _GENERIC_STAGE_TERMS
    }
    if stage_terms.intersection(role_terms):
        return True
    if _contains_prefix(stage_terms, {"train", "optimiz", "loss"}) and _contains_prefix(
        role_terms, {"train", "optimiz", "loss", "scheduler"}
    ):
        return True
    if _contains_prefix(stage_terms, {"infer", "eval", "predict"}) and _contains_prefix(
        role_terms, {"infer", "eval", "predict", "decode"}
    ):
        return True
    if _contains_prefix(stage_terms, {"data", "input", "preprocess", "feature"}) and _contains_prefix(
        role_terms, {"data", "input", "preprocess", "loader"}
    ):
        return True
    return False
...
def _mechanism_description(stage_name: str) -> str:
    normalized = _normalize_name(stage_name)
    if _contains_prefix(set(normalized.split()), {"data", "input", "preprocess", "feature"}):
        return "This stage prepares and structures inputs so downstream method components can consume them consistently."
    if _contains_prefix(set(normalized.split()), {"encode", "fusion", "interaction", "reason", "compute"}):
        return "This stage performs the core representation transformation that drives the method behavior."
    if _contains_prefix(set(normalized.split()), {"train", "optimiz", "loss", "objective", "update"}):
        return "This stage applies objective-driven optimization to update parameters using evidence-backed training logic."
    if _contains_prefix(set(normalized.split()), {"infer", "eval", "predict", "decode"}):
        return "This stage applies the learned components to produce task outputs during inference/evaluation."
    return "This stage executes an evidence-backed method transformation in the pipeline."


def _stage_inputs(stage_name: str) -> list[str]:
    normalized = _normalize_name(stage_name)
    if _contains_prefix(set(normalized.split()), {"data", "input", "preprocess", "feature"}):
        return ["raw inputs", "configuration or preprocessing settings"]
    if _contains_prefix(set(normalized.split()), {"train", "optimiz", "loss", "objective", "update"}):
        return ["model outputs", "target signals", "optimization settings"]
    if _contains_prefix(set(normalized.split()), {"infer", "eval", "predict", "decode"}):
        return ["stage representations", "model states"]
    return ["inputs from previous stage"]


def _stage_outputs(stage_name: str) -> list[str]:
    normalized = _normalize_name(stage_name)
    if _contains_prefix(set(normalized.split()), {"data", "input", "preprocess", "feature"}):
        return ["normalized representations", "ready-to-consume stage inputs"]
    if _contains_prefix(set(normalized.split()), {"train", "optimiz", "loss", "objective", "update"}):
        return ["updated parameters", "training diagnostics"]
    if _contains_prefix(set(normalized.split()), {"infer", "eval", "predict", "decode"}):
        return ["predictions", "evaluation outputs"]
    return ["outputs for next stage"]
...
def _method_name(alignment: CodeAlignmentIR, code_method_analysis: CodeMethodAnalysis | None = None) -> str:
    project_hint = _project_title_hint(alignment.project_id)
    if project_hint:
        return f"{project_hint} Method Pipeline"
    if code_method_analysis and code_method_analysis.author_alignment.author_proposed_flow:
        flow_names = [
            str(name).strip()
            for name in code_method_analysis.author_alignment.author_proposed_flow
            if str(name).strip()
        ]
        if flow_names:
            return f"{_human_stage_name(flow_names[0])} Method Pipeline"
...
def _contains_prefix(terms: set[str], prefixes: set[str]) -> bool:
    return any(term.startswith(prefix) for term in terms for prefix in prefixes)


def _project_title_hint(project_id: str) -> str:
    normalized = _normalize_name(project_id)
    if not normalized:
        return ""
    words = [word for word in normalized.split() if word and word not in {"repo", "project", "main", "src"}][:4]
    if not words:
        return ""
