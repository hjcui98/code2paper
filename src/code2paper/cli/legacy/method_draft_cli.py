"""CLI for Phase 5 method draft writing."""
...
from code2paper.authoring.writing.method_writer import build_method_draft_from_files
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--equations-tex")
    parser.add_argument("--out-md")
    parser.add_argument("--out-tex")
    args = parser.parse_args(argv)

    markdown, tex = build_method_draft_from_files(
        args.method_evidence_path,
        claim_map_path=args.claim_map,
        supplemental_equations_tex_path=args.equations_tex,
    )
    if args.out_md:
