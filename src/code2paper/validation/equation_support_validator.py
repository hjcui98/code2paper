...
from code2paper.core.schemas import MethodEvidence, Severity
...
def validate_equation_support(
    *,
    ...
    supported.extend((equation, ["supplemental_equations_tex"]) for equation in _extract_latex_equations(supplemental_equations_tex))
...
def _extract_markdown_equations(markdown: str) -> list[str]:
    ...
    for raw_line in markdown.splitlines():
        ...
        if in_block:
            ...
            non_display_lines.append(raw_line)
    non_display_markdown = "\n".join(non_display_lines)
    equations.extend(
        match.strip()
        for match in re.findall(r"(?<!\$)\$([^$\n]+?)(?<!\$)\$(?!\$)", non_display_markdown)
        if _looks_like_standalone_inline_equation(match)
    )
    return [equation for equation in equations if equation]


def _looks_like_standalone_inline_equation(value: str) -> bool:
    text = str(value or "").strip()
    if not text:
        return False
    if len(text) <= 3 and re.fullmatch(r"\\?[A-Za-z_][A-Za-z0-9_{}\\]*", text):
        return False
    if "\\cdot" in text and "=" not in text:
        return False
    return any(token in text for token in ("=", "\\sum", "\\min", "\\operatorname", "\\frac", "\\lVert"))
