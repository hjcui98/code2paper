...
class MethodPlanEquation(StrictModel):
    equation_id: str
    purpose: str
    source_family: str = ""
    required: bool = True
...
class MethodPlanSection(StrictModel):
    section_id: str
    heading: str
    purpose: str
    role: str = "core"
    ...
    input_operation_output: str = ""
    input_representation: str = ""
    operation: str = ""
    output_representation: str = ""
    figure_role: str = "include"
    priority: int = 50
    required_equation_ids: list[str] = Field(default_factory=list)
    forbidden_details: list[str] = Field(default_factory=list)


class MethodPlanOutput(StrictModel):
    method_family: str = ""
    overview_focus: str
    baseline_problem: str = ""
    method_response: str = ""
    figure_module_names: list[str] = Field(default_factory=list)
    sections: list[MethodPlanSection] = Field(default_factory=list)
    equations: list[MethodPlanEquation] = Field(default_factory=list)
    global_forbidden_details: list[str] = Field(default_factory=list)
...
class Phase5CriticIssue(StrictModel):
    issue_id: str
    severity: Severity = Severity.MEDIUM
    category: str
    message: str
...
class SelfCriticReport(StrictModel):
    issues: list[Phase5CriticIssue] = Field(default_factory=list)


class Phase5BlockedReport(StrictModel):
    project_id: str
    blocked_reason: str
    generated_prompt_artifacts: list[str] = Field(default_factory=list)


class Phase5Manifest(StrictModel):
    project_id: str
    mode: str = "blocked"
    llm_available: bool = False
    blocked_reason: str = ""
