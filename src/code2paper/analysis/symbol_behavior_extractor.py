...
def _find_symbol_node(tree: ast.Module, symbol: str) -> ast.AST | None:
    ...
    if method_matches:
        # A role such as "forward" often refers to several class methods in an
        # architecture file. Use the full module as a conservative context so
...
def _extract_architecture_parameters(
    *,
    ...
    if init_node is None or not isinstance(init_node, ast.FunctionDef):
        return []
...
def _extract_tensor_roles(
    *,
    ...
    if forward_node is None or not isinstance(forward_node, ast.FunctionDef):
        return []
    role_by_name = {
