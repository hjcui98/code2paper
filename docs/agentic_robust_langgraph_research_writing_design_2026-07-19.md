...
| `semantic_hint` | README、Markdown、论文草稿、TeX、PDF、纯文本 | 生成搜索查询、术语别名和 mismatch check；禁止直接支持正向 Method claim |
| `author_intent` | 作者 YAML | 决定研究优先级、组织偏好和待验证义务；禁止替代代码证据 |

任何最终实现句必须绑定 `executable_hard` span，必要时再绑定 call/data/control/config relation。`semantic_hint` 即使与原论文完全一致，也不能升级为硬证据。
...
Gemma vLLM/CUDA 服务属于独立模型服务环境 `/data1/users/cuihengjia/.conda_envs/gemma4-vllm-cu128`。研究 Agent 通过本地 API 调用它；编排环境与模型服务环境不合并，以避免 CUDA/vLLM 依赖污染通用 Agent 工具和测试环境。
...
| `draft_method_text` | 调用 Gemma writer |
...
- CPU 检索工具可以并发；Gemma 调用保持单实例串行，避免双卡 TP=2 资源竞争。
...
Method planner 和 writer 不应机械复刻 YAML 标题。LLM 应在授权边界内：
...
- LLM proposal parse 失败时使用 deterministic fallback；
...
4. 所有正向正文句均能回溯到 executable hard evidence；
5. 代码缺失内容以 explicit gap 终结，不进入正文；
6. 修复是 issue-scoped、可恢复、质量不回退；
7. Gemma 负责语义判断和论文组织，但不能越过证据边界；
8. RAP、EBCAR、DyG-Mamba、LinearRAG 串行 Gemma 全流程通过各自代码主线验收；
