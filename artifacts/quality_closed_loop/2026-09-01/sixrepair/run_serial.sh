...
STAMP="20260901"
TAG="sixrepair"
BASE="/tmp/c2p-${TAG}-8006-${STAMP}"
LOG="${BASE}/serial.log"
cd "$ROOT"
export PYTHONUNBUFFERED=1
{
  echo "===== SERIAL START $(date -Iseconds) ====="
  echo "repair Candidate-first Method surface (six root causes)"
  python -c "from scripts.run_authoring_replay import _code_state_digest; print('code_state_digest', _code_state_digest())"
...
run_one() {
  ...
  local fresh="/tmp/c2p-${TAG}-8006-${name}-${STAMP}"
  mkdir -p "$fresh"
...
run_one ebcar \
  ...
  "c2p-sixrepair-8006-ebcar-20260901"
...
run_one dyg \
  ...
  "c2p-sixrepair-8006-dyg-20260901"
...
run_one linearrag \
  ...
  "c2p-sixrepair-8006-linearrag-20260901"
