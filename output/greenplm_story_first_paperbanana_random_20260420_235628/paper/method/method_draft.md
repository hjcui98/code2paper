...
## Overview
<!-- c2p: stage=ALL; mechanisms=MECH1,MECH2,MECH3,MECH4; evidence=E7,E5,E4,E6,E8,E9,E10; confidence=high -->
Author-Marker Grounded Method Pipeline is designed to show that the model can use large-scale text-based alignment first, then use limited 3D data to connect point-cloud representations to the language model. Training loop type: standard. The paper-facing method is organized into 4 evidence-backed stages: Stage I: Text-First Interface Warmup, Stage II: Rich Instruction Alignment, Stage III: Point-Language Transfer, and Runtime and saving behavior.

## Evidence-Grounded Pipeline
<!-- c2p: stage=S1; mechanisms=MECH1; evidence=E7; confidence=high -->
**Stage I: Text-First Interface Warmup.** This stage is designed to establish projector-language coupling under mostly frozen backbone settings. It consumes raw point cloud and produces the next stage inputs.

<!-- c2p: stage=S2; mechanisms=MECH2; evidence=E7; confidence=high -->
...
**Stage III: Point-Language Transfer.** This stage is designed to use point-cloud token features and compact token aggregation to transfer prior alignment to 3D inputs. It consumes the previous stage outputs and produces the next stage inputs. The stage uses PointcloudEncoder for injecting projected point features into language token stream. PointcloudEncoder # grouper # define the encoder. The data flow links input to PointcloudEncoder via forward and PointcloudEncoder to output via output.

<!-- c2p: stage=S4; mechanisms=MECH4; evidence=E7; confidence=high -->
**Runtime and saving behavior.** This stage is designed to control runtime mechanics (trainer save behavior/logging/checkpoint flow). It consumes the previous stage outputs and produces the next stage inputs.

## Core Components
<!-- c2p: stage=ALL; mechanisms=MECH1,MECH2,MECH3,MECH4; evidence=E7,E5,E4,E6,E8,E9,E10; confidence=high -->
The core method components support the following paper-facing roles: inject projected point features into language token stream. Utility and experiment-support modules are not treated as method innovations.
